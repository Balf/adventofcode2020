...
public class Day4 {
    ...
    private static final String input = "byr:1985\n" +
    ...
    private static final String[] eyeColours = new String[]{"amb", "blu", "brn", "gry", "grn", "hzl", "oth"};
    ...
    public static void main(String[] args) {
        ...
        String[] passportLines = input.split("\\n\\n");

        for (String passportLine : passportLines) {

            boolean isValid = new Passport(passportLine).isValid();

            if (new Passport(passportLine).isValid()) {
                validPassports = validPassports + 1;
            }

            allPassports++;

            System.out.println("Passport number " + allPassports + " is " + isValid);
        }
    ...
    static class Passport {
        ...
        Passport(String passport) {
            String[] keyValuePairs = passport.split("\\s");

            for (String keyValuePair : keyValuePairs) {

                if (keyValuePair.contains("byr")) {
                    this.byr = Integer.parseInt(keyValuePair.substring(keyValuePair.indexOf(":") + 1));
                }

                if (keyValuePair.contains("iyr")) {
                    this.iyr = Integer.parseInt(keyValuePair.substring(keyValuePair.indexOf(":") + 1));
                }

                if (keyValuePair.contains("eyr")) {
                    this.eyr = Integer.parseInt(keyValuePair.substring(keyValuePair.indexOf(":") + 1));
                }

                if (keyValuePair.contains("hgt")) {
                    String value = keyValuePair.substring(keyValuePair.indexOf(":") + 1);
                    if (value.contains("cm")) {
                        value = value.replace("cm", "");
                        this.hgt = Integer.parseInt(value);
                        this.hgtUnit = "cm";
                ...
                if (keyValuePair.contains("ecl")) {
                    this.ecl = keyValuePair.substring(keyValuePair.indexOf(":") + 1);
                }

                if (keyValuePair.contains("hcl")) {
                    this.hcl = keyValuePair.substring(keyValuePair.indexOf(":") + 1);
                }

                if (keyValuePair.contains("pid")) {
                    this.pid = keyValuePair.substring(keyValuePair.indexOf(":") + 1);
                }
            }
        }
        ...
        boolean isByrValid() {
            //System.out.println("byr: " + byr + " is valid: " + (byr >= 1920 && byr <= 2002));
            return byr >= 1920 && byr <= 2002;
        }

        boolean isIyrValid() {
            //System.out.println("iyr: " + iyr + " is valid: " + (iyr >= 2010 && iyr <= 2020));
        ...
        boolean isHeightValid() {
            if (hgtUnit != null && hgtUnit.equals("cm")) {
                return this.hgt >= 150 && this.hgt <= 193;
            } else if (hgtUnit != null && hgtUnit.equals("in")) {
                return this.hgt >= 59 && this.hgt <= 76;
            }
        ...
        boolean isEclValid() {
            return Arrays.asList(eyeColours).contains(this.ecl);
        }
