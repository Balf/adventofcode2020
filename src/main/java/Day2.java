...
class Day2 {
    ...
    private static final String input = "1-4 n: nnnnn\n" +
            "5-7 z: qhcgzzz\n" +
            "7-11 m: mmmmmmsmmmmm\n" +
    ...
    private static void part1() {
        String[] policyLines = input.split("\\n");

        int validPasswords = 0;

        for (String policyLine : policyLines) {
            Map<String, String> policyProperties = getPolicy(policyLine);

            String password = policyProperties.get("password");
            String character = policyProperties.get("character");

            long count = password.chars().filter(ch -> ch == character.charAt(0)).count();
            int minCount = Integer.parseInt(policyProperties.get("minCount"));
            int maxCount = Integer.parseInt(policyProperties.get("maxCount"));
    ...
    private static void part2() {
        ...
        for (String policyLine : policyLines) {
            ...
            //Subtract one to compensate for non-zero based indexes
            int firstIndex = Integer.parseInt(policyProperties.get("minCount")) - 1;
            int secondIndex = Integer.parseInt(policyProperties.get("maxCount")) - 1;
            String password = policyProperties.get("password");
            String character = policyProperties.get("character");

            String firstCharacter = String.valueOf(password.charAt(firstIndex));
            String secondCharacter = String.valueOf(password.charAt(secondIndex));

            if (firstCharacter.equals(character) ^ secondCharacter.equals(character)) {
                validPasswords++;
            }
    ...
    private static Map<String, String> getPolicy (String policyLine) {

        String policy = policyLine.substring(0, policyLine.indexOf(":"));
        String password = policyLine.substring(policyLine.indexOf(":") + 2);

        String minCount = policy.substring(0,policy.indexOf("-"));
        String maxCountString = policy.substring(policy.indexOf("-"));
        String maxCount = maxCountString.substring(1, maxCountString.indexOf(" "));

        Map<String, String> policyProperties = new HashMap<>();

        policyProperties.put("minCount", minCount);
        policyProperties.put("maxCount", maxCount);
        policyProperties.put("character", policy.substring(policy.length() - 1));
        policyProperties.put("password", password);
